...
namespace Moosend.Api.Client
{
    public partial class MoosendApiClient
    {
        ...
        /// <param name="token"> Cancellation Token. </param>
        /// <returns></returns>
        public async Task<Campaign> GetCampaignAsync(Guid campaignId, CancellationToken token = default(CancellationToken))
        {
            var path = string.Format("/campaigns/{0}/view", campaignId);

            return await SendAsync<Campaign>(HttpMethod.Get, path, null, token).ConfigureAwait(false);
        ...
        /// <param name="token"> Cancellation Token. </param>
        /// <returns></returns>
        public async Task<Campaign> CloneAsync(Guid campaignId, CancellationToken token = default(CancellationToken))
        {
            return await SendAsync<Campaign>(HttpMethod.Post, string.Format("/campaigns/{0}/clone", campaignId), null, token).ConfigureAwait(false);
        }
        ...
        /// <param name="token"> Cancellation Token. </param>
        /// <returns></returns>
        public async Task<CampaignStatisticsSummary> GetCampaignSummaryAsync(Guid campaignId, CancellationToken token = default(CancellationToken))
        {
            var path = string.Format("/campaigns/{0}/view_summary", campaignId);

            return await SendAsync<CampaignStatisticsSummary>(HttpMethod.Get, path, null, token).ConfigureAwait(false);
