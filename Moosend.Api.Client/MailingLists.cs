...
namespace Moosend.Api.Client
{
    public partial class MoosendApiClient
    {
        ...
        /// </param>
        /// <param name="token"> Cancellation Token. </param>
        public async Task<SubscribersResponse> GetSubscribersAsync(Guid mailingListId, SubscribeType? status, DateTime? since = null, int page = 1, int pageSize = 500, CancellationToken token = default(CancellationToken))
        {
            var parameters = new
            {
                Since = since,
            ...
            return await SendAsync<SubscribersResponse>(HttpMethod.Get, path, parameters, token).ConfigureAwait(false);
        }
