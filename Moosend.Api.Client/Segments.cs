...
namespace Moosend.Api.Client
{
    public partial class MoosendApiClient
    {
        ...
        public async Task<SegmentsResponse> GetSegmentsForListAsync(Guid mailingListId, int page = 1, int pageSize = 100, CancellationToken token = default(CancellationToken))
        {
            var parameters = new
            {
                Page = page,
            ...
            return await SendAsync<SegmentsResponse>(HttpMethod.Get, string.Format("/lists/{0}/segments", mailingListId), parameters, token).ConfigureAwait(false);
        }
        ...
        /// <param name="token"> Cancellation Token. </param>
        /// <returns></returns>
        public async Task<SubscribersResponse> GetSegmentSubscribersAsync(Guid mailingListId, int segmentId, SubscribeType? status, int page = 1, int pageSize = 100, CancellationToken token = default(CancellationToken))
        {
            ...
            return await SendAsync<SubscribersResponse>(HttpMethod.Get, string.Format("/lists/{0}/segments/{1}/members", mailingListId, segmentId), parameters, token).ConfigureAwait(false);
        }
    }
}