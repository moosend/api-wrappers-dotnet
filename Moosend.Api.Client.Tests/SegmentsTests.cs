...
namespace Moosend.Api.Tests
{
    ...
    public class SegmentsTests
    {
        ...
        public async Task Given_MoosendApiClient_When_Getting_Segments_Then_Can_Get_Segment()
        {
            ...
            var expectedSegments = new SegmentsResponse()
            {
                Paging = new Paging() { TotalResults = 1 },
                Segments = new List<Segment>() { new Segment() { Id = segId } }
            };

            var content = new ApiResponse<SegmentsResponse>() { Context = expectedSegments };
        ...
        public async Task Given_MoosendApiClient_When_Getting_Segment_Members_Then_Can_Get_Members()
        {
            ...
            var expectedSubscribers = new SubscribersResponse()
            {
                Paging = new Paging() { TotalResults = 1 },
                Subscribers = new List<Subscriber>() { new Subscriber() { Id = subId } }
            };

            var content = new ApiResponse<SubscribersResponse>() { Context = expectedSubscribers };
            ctx.Handler = new TestHttpMessageHandler(HttpStatusCode.OK, content);
