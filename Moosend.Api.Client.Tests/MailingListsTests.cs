...
namespace Moosend.Api.Tests
{
    ...
    public class MailingListsTests
    {
        ...
        public void Setup()
        {
            var endpoint = ConfigurationManager.AppSettings["edge.moosend-api-client.endpoint"];
        ...
        public async Task Given_MoosendApiClient_When_Getting_Mailing_Lists_Then_Can_Get_Mailing_Lists()
        {
            ...
            var expectedLists = new MailingListsResponse()
            {
                Paging = new Paging() { TotalResults = 1 },
                MailingLists = new List<MailingList>() { new MailingList() { Id = listId } }
            };

            var content = new ApiResponse<MailingListsResponse>() { Context = expectedLists };
        ...
        public async Task Given_MoosendApiClient_When_Updating_List_Then_The_Right_Url_Is_Accessed()
        {
            ...
            var url = string.Format("/lists/{0}/update.json?apikey={1}", listId, _apiKey);

            var expectedUrl = new Uri(_uri + url);

            try
            {
                await _client.UpdateMailingListAsync(listId, "list_name");
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Updating_List_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.UpdateMailingListAsync(Guid.NewGuid(), "list_name");
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Updating_List_Then_Can_Update()
        {
            ...
            var content = new ApiResponse<Guid> { Context = Guid.NewGuid() };
            ctx.Handler = new TestHttpMessageHandler(HttpStatusCode.OK, content);

            var client = new MoosendApiClient(_apiKey, ctx);

            // act
            var listId = await client.UpdateMailingListAsync(Guid.NewGuid(), "list_name");
        ...
        public async Task Given_MoosendApiClient_When_Updating_List_Then_The_Payload_Is_As_Expected()
        {
            ...
            try
            {
                await _client.UpdateMailingListAsync(Guid.NewGuid(), listName, confirmationPage, redirectAfterUnsubscribePage);
        ...
        public async Task Given_MoosendApiClient_When_Getting_A_Mailing_List_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.GetMailingListByIdAsync(Guid.NewGuid());
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Getting_A_Mailing_List_Then_Can_Get_Mailing_List()
        {
            ...
            // act
            var listResult = await client.GetMailingListByIdAsync(Guid.NewGuid());
        ...
        public async Task Given_MoosendApiClient_When_Getting_Subscribers_List_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.GetSubscribersAsync(Guid.NewGuid(), null);
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Getting_Subscribers_List_Then_Can_Get_Subscribers()
        {
            ...
            var expectedResult = new SubscribersResponse
            {
                Paging = new Paging() { TotalResults = 200},
                Subscribers = new List<Subscriber>() {new Subscriber() {Id = Guid.NewGuid()}}
            };

            var expectedContent = new ApiResponse<SubscribersResponse>() { Context = expectedResult };

            ctx.Handler = new TestHttpMessageHandler(HttpStatusCode.OK, expectedContent);
            var client = new MoosendApiClient(_apiKey, ctx);

            // act
            var result = await client.GetSubscribersAsync(Guid.NewGuid(), null);
        ...
        public async Task Given_MoosendApiClient_When_Deleting_A_Mailing_List_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.DeleteMailingListAsync(Guid.NewGuid());
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Deleting_A_Mailing_List_Then_Can_Delete_Mailing_List()
        {
            ...
            // act
            var success = await client.DeleteMailingListAsync(Guid.NewGuid());
        ...
        public async Task Given_MoosendApiClient_When_Creating_Custom_Field_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.CreateCustomFieldAsync(Guid.NewGuid(), "cf_name");
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Creating_Custom_Field_Then_Can_Create()
        {
            ...
            // act
            var cfId = await client.CreateCustomFieldAsync(Guid.NewGuid(), "cf_name");
        ...
        public async Task Given_MoosendApiClient_When_Creating_Custom_Field_Then_The_Payload_Is_As_Expected()
        {
            ...
            try
            {
                await _client.CreateCustomFieldAsync(Guid.NewGuid(), name, type, isRequired, options);
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Updating_Custom_Field_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.UpdateCustomFieldAsync(Guid.NewGuid(), Guid.NewGuid(), "cf_name");
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        public async Task Given_MoosendApiClient_When_Updating_Custom_Field_Then_Can_Create()
        {
            ...
            // act
            var cfId = await client.UpdateCustomFieldAsync(Guid.NewGuid(), Guid.NewGuid(), "cf_name");
        ...
        [Test]
        public async Task Given_MoosendApiClient_When_Deleting_Custom_Field_Then_The_Right_Url_Is_Accessed()
        {
            var listId = Guid.NewGuid();
            var cfId = Guid.NewGuid();
            var url = string.Format("/lists/{0}/customfields/{1}/delete.json?apikey={2}", listId, cfId, _apiKey);

            var expectedUrl = new Uri(_uri + url);

            try
            {
                await _client.DeleteCustomFieldAsync(listId, cfId);
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        [Test]
        public async Task Given_MoosendApiClient_When_Deleting_Custom_Field_Then_The_Headers_Are_As_Expected()
        {
            try
            {
                await _client.DeleteCustomFieldAsync(Guid.NewGuid(), Guid.NewGuid());
            }
            catch (Exception ex)
            {
                // known serialization exception caused by returning {} from test handler 
        ...
        [Test]
        public async Task Given_MoosendApiClient_When_Deleting_Custom_Field_Then_Can_Create()
        {
            // arrange
            var ctx = new ServiceClientContext(_uri);
            ...
            // act
            var cfId = await client.DeleteCustomFieldAsync(Guid.NewGuid(), Guid.NewGuid());
