...
namespace Moosend.API.Client.Wrappers
{
    public class MembersWrapper
    {
        ...
        internal MembersWrapper(ApiManager manager)
        {
            _Manager = manager;
        }

        public MailingListMember GetByEmail(Guid mailingListID, String email)
        {
            return _Manager.MakeRequest<MailingListMember>(HttpMethod.GET, String.Format("/subscribers/{0}/view", mailingListID), new { Email = email });
        }

        public IList<MailingListMember> Subscribe(Guid mailingListID, IList<MailingListMemberParams> members)
        {
            return _Manager.MakeRequest<IList<MailingListMember>>(HttpMethod.POST, String.Format("/subscribers/{0}/subscribe_many", mailingListID), new { 
        ...
        public MailingListMember Subscribe(Guid mailingListID, MailingListMemberParams member)
        {
            return _Manager.MakeRequest<MailingListMember>(HttpMethod.POST, String.Format("/subscribers/{0}/subscribe", mailingListID), new
            {
                Name = member.Name,
                Email = member.Email,
